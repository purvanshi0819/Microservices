...
public class CardsController {
    ...
    private final ICardsSrvice iCardsSrvice;

    public CardsController(ICardsSrvice iCardsSrvice){
        this.iCardsSrvice=iCardsSrvice;
    }
    ...
    public ResponseEntity<ResponseDto> createCard(@RequestParam @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits") String mobileNumber){
        iCardsSrvice.createCard(mobileNumber);
    ...
    public ResponseEntity<CardsDto> fetchCardDetails(@RequestHeader("eazybank-correlation-id") String correlationId
        ...
        CardsDto cardsDto=iCardsSrvice.fetchCard(mobileNumber);
        logger.debug("fetchCardDetails method stop");
    ...
    public ResponseEntity<ResponseDto> updateCardDetails(@Valid @RequestBody CardsDto cardsDto){
        boolean isUpdated=iCardsSrvice.updateCard(cardsDto);
        if(isUpdated){
            return ResponseEntity
    ...
    public ResponseEntity<ResponseDto> deleteCardDetails(@RequestParam @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits") String mobileNumber){
        boolean isDeleted=iCardsSrvice.deleteCard(mobileNumber);
        if(isDeleted){
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(new ResponseDto(CardsConstants.STATUS_200, CardsConstants.MESSAGE_200));
